...
events.get('/', async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    `SELECT e.care_event_id, e.event_date, e.event_type, e.title, e.facility, e.notes,
            (SELECT COUNT(*) FROM timeline t WHERE t.care_event_id = e.care_event_id AND t.deleted = 0) items,
            (SELECT MIN(t.date) FROM timeline t WHERE t.care_event_id = e.care_event_id AND t.deleted = 0) first_date,
            (SELECT MAX(t.date) FROM timeline t WHERE t.care_event_id = e.care_event_id AND t.deleted = 0) last_date
       FROM care_events e
...
events.get('/:id', async (c) => {
  ...
  const event = await c.env.DB.prepare(
    `SELECT * FROM care_events WHERE care_event_id = ? AND deleted = 0`
  ).bind(id).first<any>();
  if (!event) throw new Error('That event no longer exists.');
  assertScope(c.get('caller'), event.person_id);

  const { results } = await c.env.DB.prepare(
    `SELECT kind, ref_id, date, title, value, detail, flag FROM timeline
...
events.post('/', async (c) => {
  ...
  await c.env.DB.prepare(
    `INSERT INTO care_events (care_event_id, person_id, event_date, event_type, title, facility, notes)
     VALUES (?,?,?,?,?,?,?)`
...
events.put('/:id', async (c) => {
  ...
  const row = await c.env.DB.prepare(
    `SELECT person_id FROM care_events WHERE care_event_id = ? AND deleted = 0`
  ...
  await c.env.DB.prepare(
    `UPDATE care_events SET event_date=COALESCE(?,event_date), title=COALESCE(?,title),
...
events.delete('/:id', async (c) => {
  ...
  const id = c.req.param('id');
  const row = await c.env.DB.prepare(
    `SELECT person_id FROM care_events WHERE care_event_id = ?`
  ).bind(id).first<{ person_id: string }>();
  if (!row) throw new Error('Already gone.');
  assertScope(caller, row.person_id);

  await c.env.DB.batch([
    c.env.DB.prepare(`UPDATE care_events SET deleted = 1 WHERE care_event_id = ?`).bind(id),
    c.env.DB.prepare(`UPDATE records SET care_event_id = NULL WHERE care_event_id = ?`).bind(id),
    c.env.DB.prepare(`UPDATE timeline SET care_event_id = NULL WHERE care_event_id = ?`).bind(id),
    c.env.DB.prepare(`INSERT INTO audit_log (actor, action, ref_id) VALUES (?, 'event_removed', ?)`)
...
events.post('/link', async (c) => {
  ...
  const record = await c.env.DB.prepare(
    `SELECT person_id FROM records WHERE record_id = ? AND deleted = 0`
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(`UPDATE records SET care_event_id = ?, updated_at = datetime('now') WHERE record_id = ?`)
      .bind(careEventId, recordId),
    c.env.DB.prepare(`UPDATE timeline SET care_event_id = ? WHERE ref_id = ? OR ref_id IN (
        SELECT result_id FROM test_results WHERE record_id = ?
        UNION ALL SELECT medicine_id FROM medicines WHERE record_id = ?
        UNION ALL SELECT diagnosis_id FROM diagnoses WHERE record_id = ?
        UNION ALL SELECT follow_up_id FROM follow_ups WHERE record_id = ?
        UNION ALL SELECT bill_id FROM bills WHERE record_id = ?
        UNION ALL SELECT document_id FROM documents WHERE record_id = ?)`)
...
events.post('/promote/:recordId', async (c) => {
  ...
  const r = await c.env.DB.prepare(
    `SELECT * FROM records WHERE record_id = ? AND deleted = 0`
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(`UPDATE records SET care_event_id = ? WHERE record_id = ?`).bind(id, recordId),
    c.env.DB.prepare(`UPDATE timeline SET care_event_id = ? WHERE ref_id = ? OR ref_id IN (
        SELECT result_id FROM test_results WHERE record_id = ?
        UNION ALL SELECT medicine_id FROM medicines WHERE record_id = ?
        UNION ALL SELECT diagnosis_id FROM diagnoses WHERE record_id = ?
