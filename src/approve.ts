...
export async function approveJob(
  ...
  for (const [i, f] of files.entries()) {
    ...
    stmts.push(env.DB.prepare(
      ...
      documentFileName({ person: personName, date: docDate,
        recordType: meta.document_type || data.record_type, provider,
        documentId, originalName: f.file_name }),
      key, f.mime_type, f.bytes, f.content_sha256,
