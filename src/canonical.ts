...
async function aliases(env: Env): Promise<Record<string, Alias>> {
  ...
  const { results } = await env.DB.prepare(
    `SELECT alias_key, parameter, unit FROM health_parameter_aliases`
...
async function conversions(env: Env): Promise<Record<string, { to: string; mul: number; add: number }>> {
  ...
  const { results } = await env.DB.prepare(
    `SELECT parameter, from_unit_key, to_unit, multiply_by, add_offset FROM health_unit_conversions`
  ).all<any>();
...
export async function learnAlias(
  ...
  await env.DB.prepare(
    `INSERT OR IGNORE INTO health_parameter_aliases (alias_key, original, parameter, unit, source)
     VALUES (?, ?, ?, ?, 'gemini')`
...
export async function referenceBand(
  ...
  const row = await env.DB.prepare(
    `SELECT low, high FROM health_reference_bands WHERE parameter = ? ORDER BY sex = 'any' DESC LIMIT 1`
