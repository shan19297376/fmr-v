...
const EDITABLE: Record<string, {
  ...
  test: {
    table: 'test_results', idCol: 'result_id',
  ...
  medicine: {
    table: 'medicines', idCol: 'medicine_id',
  ...
  document: {
    table: 'documents', idCol: 'document_id',
...
records.put('/:kind/:id', async (c) => {
  ...
  const t = spec.timeline(kind === 'test'
    ? await c.env.DB.prepare(`SELECT * FROM test_results WHERE result_id = ?`).bind(id).first<any>()
    : updated);

  await c.env.DB.batch([
    c.env.DB.prepare(
      `UPDATE timeline SET date=?, title=?, value=?, detail=?, flag=?, search_text=?
        WHERE ref_id = ?`
    ).bind(t.date, t.title, t.value, t.detail, t.flag,
           `${t.title} ${t.value} ${t.detail}`.toLowerCase().slice(0, 500), id),
    c.env.DB.prepare(
      `INSERT INTO corrections (correction_id, actor, person_id, table_name, row_id, before_json, after_json, reason)
       VALUES (?,?,?,?,?,?,?,?)`
...
records.delete('/:kind/:id', async (c) => {
  ...
  await c.env.DB.batch([
    ...
    c.env.DB.prepare(`UPDATE timeline SET deleted = 1 WHERE ref_id = ?`).bind(id),
    c.env.DB.prepare(`INSERT INTO audit_log (actor, action, ref_id, detail) VALUES (?, 'deleted', ?, ?)`)
...
async function recomputeTest(env: Env, t: any): Promise<void> {
  ...
  await env.DB.prepare(
    `UPDATE test_results SET parameter=?, unit=?, value_a=?, value_b=?, ref_low=?, ref_high=?, is_abnormal=?
      WHERE result_id = ?`
...
records.post('/followup/:id/:action', async (c) => {
  ...
  const row = await c.env.DB.prepare(
    `SELECT person_id, type, instruction FROM follow_ups WHERE follow_up_id = ? AND deleted = 0`
  ).bind(id).first<any>();
  ...
  await c.env.DB.batch([
    dueDate
      ? c.env.DB.prepare(`UPDATE follow_ups SET status=?, due_date=? WHERE follow_up_id = ?`).bind(status, dueDate, id)
      : c.env.DB.prepare(`UPDATE follow_ups SET status=? WHERE follow_up_id = ?`).bind(status, id),
    c.env.DB.prepare(`UPDATE timeline SET flag=?, date=COALESCE(?, date) WHERE ref_id = ?`).bind(status, dueDate, id),
    c.env.DB.prepare(`INSERT INTO audit_log (actor, action, ref_id, detail) VALUES (?, ?, ?, ?)`)
...
records.post('/reading', async (c) => {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(
      `INSERT INTO records (record_id, person_id, care_event_id, event_date, record_type, facility, summary, key_findings)
       VALUES (?,?,?,?,'Manual Reading',?,?,?)`
    ).bind(recordId, b.person, b.careEventId ?? null, date, context, summary, b.notes ?? ''),
    c.env.DB.prepare(
      `INSERT INTO test_results (result_id, record_id, person_id, test_date, panel, parameter_raw, parameter,
...
records.get('/aliases/list', async (c) => {
  const { results } = await c.env.DB.prepare(
    `SELECT a.alias_key, a.original, a.parameter, a.unit, a.source,
            (SELECT COUNT(*) FROM test_results t WHERE t.parameter = a.parameter AND t.deleted = 0) uses
       FROM parameter_aliases a
      WHERE a.source <> 'seed'
...
records.put('/aliases/:key', async (c) => {
  ...
  const existing = await c.env.DB.prepare(
    `SELECT parameter FROM parameter_aliases WHERE alias_key = ?`
  ).bind(key).first<{ parameter: string }>();
  if (!existing) throw new Error('That mapping no longer exists.');

  await c.env.DB.batch([
    c.env.DB.prepare(
      `UPDATE parameter_aliases SET parameter=?, unit=?, source='manual', updated_at=datetime('now')
        WHERE alias_key = ?`
    ).bind(clean, String(unit ?? ''), key),
    c.env.DB.prepare(
      `UPDATE test_results SET parameter = ? WHERE parameter = ? AND deleted = 0`
    ).bind(clean, existing.parameter),
    c.env.DB.prepare(
      `UPDATE timeline SET title = ? WHERE kind = 'test' AND title = ?`
    ).bind(clean, existing.parameter),
    c.env.DB.prepare(`INSERT INTO audit_log (actor, action, ref_id, detail) VALUES (?, 'alias_edited', ?, ?)`)
