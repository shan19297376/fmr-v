...
care.get('/medicines', async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    `SELECT m.*, r.event_date, r.doctor, r.facility
       FROM medicines m LEFT JOIN records r USING (record_id)
...
care.post('/medicines', async (c) => {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(
      `INSERT INTO medicines (medicine_id, person_id, prescribed_on, name, strength, dose,
    ...
    c.env.DB.prepare(
      `INSERT INTO timeline (entry_id, person_id, kind, ref_id, date, title, value, detail, flag, search_text)
       VALUES (?,?,'medicine',?,?,?,?,?,?,?)`
...
care.post('/medicines/:id/:action', async (c) => {
  ...
  const m = await c.env.DB.prepare(
    `SELECT person_id, name, end_date FROM medicines WHERE medicine_id = ? AND deleted = 0`
  ).bind(id).first<any>();
  ...
  await c.env.DB.batch([
    c.env.DB.prepare(`UPDATE medicines SET status = ?, end_date = ? WHERE medicine_id = ?`)
      .bind(status, endDate, id),
    c.env.DB.prepare(`UPDATE timeline SET flag = ? WHERE ref_id = ?`).bind(status, id),
...
care.get('/reminders', async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    `SELECT r.*, p.name person FROM reminders r JOIN people p USING (person_id)
...
care.post('/reminders', async (c) => {
  ...
  await c.env.DB.prepare(
    `INSERT INTO reminders (reminder_id, person_id, kind, title, detail, due_date, repeat_days, created_by)
     VALUES (?,?,'custom',?,?,?,?,?)`
...
care.post('/reminders/:id/:action', async (c) => {
  ...
  const r = await c.env.DB.prepare(`SELECT * FROM reminders WHERE reminder_id = ?`)
    .bind(id).first<any>();
  ...
  if (action === 'snooze') {
    ...
    await c.env.DB.prepare(`UPDATE reminders SET due_date = ? WHERE reminder_id = ?`)
  ...
  await c.env.DB.prepare(`UPDATE reminders SET status = ? WHERE reminder_id = ?`)
  ...
  if (action === 'done' && r.kind === 'followup' && r.source_ref) {
    await c.env.DB.batch([
      c.env.DB.prepare(`UPDATE follow_ups SET status = 'completed' WHERE follow_up_id = ?`).bind(r.source_ref),
      c.env.DB.prepare(`UPDATE timeline SET flag = 'completed' WHERE ref_id = ?`).bind(r.source_ref),
    ]);
  }
...
export async function upsertReminder(env: Env, r: {
  ...
  await env.DB.prepare(
    `INSERT INTO reminders (reminder_id, person_id, kind, title, detail, due_date, repeat_days, source_ref)
     VALUES (?,?,?,?,?,?,?,?)
...
export async function refreshReminders(env: Env): Promise<{ created: number }> {
  ...
  const [follow, meds] = await env.DB.batch([
    env.DB.prepare(
      `SELECT follow_up_id, person_id, due_date, type, instruction FROM follow_ups
        WHERE deleted = 0 AND status = 'pending' AND due_date IS NOT NULL AND due_date <= ?`
    ).bind(soon),
    env.DB.prepare(
      `SELECT medicine_id, person_id, name, end_date FROM medicines
  ...
  await env.DB.prepare(
    `UPDATE reminders SET status = 'done'
      WHERE status = 'pending' AND kind = 'followup' AND source_ref IN
        (SELECT follow_up_id FROM follow_ups WHERE status <> 'pending' OR deleted = 1)`
  ).run();
...
export async function nextDue(env: Env, personId: string): Promise<any | null> {
  return env.DB.prepare(
    `SELECT reminder_id, kind, title, detail, due_date FROM reminders
