...
import { care, refreshReminders, nextDue } from './care';
import { fileJob, tryAutoFile } from './autofile';
...
async function scopeFor(env: Env, email: string): Promise<Caller> {
  const row = await env.DB.prepare(
    `SELECT role, scope_person_id, expires_at FROM core_app_users WHERE email = ?`
  ...
  if (!row) {
    const { count } = await env.DB.prepare(
      `SELECT COUNT(*) AS count FROM core_app_users`
    ).first<{ count: number }>() ?? { count: 0 };

    if (count === 0) {
      await env.DB.prepare(
        `INSERT INTO core_app_users (email, role, display_name) VALUES (?, 'owner', ?)`
...
app.use('/api/*', async (c, next) => {
  ...
  c.executionCtx.waitUntil(
    c.env.DB.prepare(`UPDATE core_app_users SET last_seen_at = datetime('now') WHERE email = ?`)
      .bind(email).run()
  );
  await next();
});
...
app.get('/api/core/people', async (c) => {
 ...
 */
app.get('/api/health/timeline', async (c) => {
  const caller = c.get('caller');
...
app.get('/api/health/trends', async (c) => {
...
app.get('/api/health/trends/series', async (c) => {
  const person = requirePerson(c.get('caller'), c.req.query('person'));
  ...
  const band = await c.env.DB.prepare(
    `SELECT low, high, unit FROM health_reference_bands
...
app.get('/api/health/snapshot', async (c) => {
  ...
  const [profile, meds, followUps, abnormal, lastVisit] = await c.env.DB.batch([
    ...
    c.env.DB.prepare(
      `SELECT event_date, record_type, summary, facility FROM health_records
...
app.post('/api/core/uploads', async (c) => {
  const caller = c.get('caller');
 ...
 */
app.post('/api/core/uploads/:jobId/file', async (c) => {
  const caller = c.get('caller');
  if (caller.role === 'viewer') throw new HttpError(403, 'Read-only access.');
  ...
  const dupe = await c.env.DB.prepare(
    `SELECT document_id, person_id, document_date, file_name FROM core_documents
...
app.get('/api/core/uploads/:jobId/file/:index', async (c) => {
  ...
  const file = await c.env.DB.prepare(
    `SELECT r2_key, mime_type, file_name FROM core_job_files
      WHERE job_id = ? ORDER BY file_index LIMIT 1 OFFSET ?`
  ).bind(jobId, Math.max(0, Number(c.req.param('index')) || 0)).first<any>();
  if (!file) throw new HttpError(404, 'No such file on this upload.');

  return new Response(await getDocument(c.env, file.r2_key), {
    headers: {
...
app.post('/api/core/people', async (c) => {
  const caller = c.get('caller');
  ...
  const exists = await c.env.DB.prepare(
    `SELECT 1 FROM core_people WHERE lower(name) = lower(?)`
  ).bind(clean).first();
  ...
  await c.env.DB.batch([
    ...
    c.env.DB.prepare(`INSERT INTO core_profiles (person_id) VALUES (?)`).bind(personId),
    c.env.DB.prepare(
      `INSERT INTO core_audit_log (actor, action, ref_id, detail) VALUES (?, 'person_added', ?, ?)`
...
app.post('/api/core/uploads/:jobId/approve', async (c) => {
  const caller = c.get('caller');
  if (caller.role === 'viewer') throw new HttpError(403, 'Read-only access.');
 ...
 */
app.get('/api/health/panel', async (c) => {
  const person = requirePerson(c.get('caller'), c.req.query('person'));
  ...
  const { results } = await c.env.DB.prepare(
    `SELECT t.parameter, t.test_date, t.result_text, t.unit_raw, t.value_a,
       ...
       FROM health_test_results t
       LEFT JOIN health_test_categories tc ON tc.parameter = t.parameter
...
app.get('/api/health/panels', async (c) => {
  const person = requirePerson(c.get('caller'), c.req.query('person'));
  const { results } = await c.env.DB.prepare(
    `SELECT COALESCE(tc.category,'Other tests') category,
       ...
       FROM health_test_results t LEFT JOIN health_test_categories tc ON tc.parameter = t.parameter
      WHERE t.person_id = ? AND t.deleted = 0
...
/** The apps this account may open. One today; the shell is built for more. */
app.get('/api/core/apps', async (c) => {
  const caller = c.get('caller');
  const { results } = await c.env.DB.prepare(
    `SELECT a.app_id, a.name, a.tagline, a.icon,
            COALESCE(x.role, CASE WHEN ? = 'owner' THEN 'owner' ELSE NULL END) role
       FROM core_apps a
       LEFT JOIN core_app_access x ON x.app_id = a.app_id AND x.email = ?
      WHERE a.enabled = 1 ORDER BY a.sort_order`
  ).bind(caller.role, caller.email).all<any>();
  return c.json(results.filter((a) => a.role));
});
...
app.put('/api/core/people/:id', async (c) => {
  const caller = c.get('caller');
  ...
  const clash = await c.env.DB.prepare(
    `SELECT person_id FROM core_people WHERE lower(name) = lower(?) AND person_id <> ?`
  ).bind(clean, id).first();
  if (clash) throw new HttpError(409, `${clean} already exists. Merge them instead.`);

  await c.env.DB.batch([
    c.env.DB.prepare(`UPDATE core_people SET name = ? WHERE person_id = ?`).bind(clean, id),
    c.env.DB.prepare(`INSERT INTO core_audit_log (actor, action, ref_id, detail) VALUES (?, 'person_renamed', ?, ?)`)
 ...
 */
app.post('/api/core/people/:id/merge', async (c) => {
  const caller = c.get('caller');
  ...
  const tables = ['health_records', 'core_documents', 'health_test_results', 'health_medicines',
                  'health_diagnoses', 'health_follow_ups', 'health_bills', 'health_timeline',
                  'health_care_events', 'core_jobs'];
  await c.env.DB.batch([
    ...tables.map((t) => c.env.DB.prepare(`UPDATE ${t} SET person_id = ? WHERE person_id = ?`).bind(into, from)),
    c.env.DB.prepare(`DELETE FROM core_profiles WHERE person_id = ?`).bind(from),
    c.env.DB.prepare(`UPDATE core_people SET active = 0 WHERE person_id = ?`).bind(from),
    c.env.DB.prepare(`INSERT INTO core_audit_log (actor, action, ref_id, detail) VALUES (?, 'person_merged', ?, ?)`)
...
/** What the app filed on its own, newest first — for a quick sanity check. */
app.get('/api/core/filed', async (c) => {
  const s = scopeClause(c.get('caller'), 'r.person_id');
  const { results } = await c.env.DB.prepare(
    `SELECT j.job_id, j.filed_at, j.detected_name, r.record_id, r.event_date, r.record_type,
            r.summary, r.facility, p.name person,
            e.care_event_id, e.title episode, e.auto_created,
            (SELECT COUNT(*) FROM health_test_results t WHERE t.record_id = r.record_id AND t.deleted = 0) tests,
            (SELECT COUNT(*) FROM health_medicines m WHERE m.record_id = r.record_id AND m.deleted = 0) medicines,
            (SELECT COUNT(*) FROM health_bills b WHERE b.record_id = r.record_id AND b.deleted = 0) bills
       FROM core_jobs j
       JOIN health_records r ON r.record_id = j.record_id
       JOIN core_people p ON p.person_id = r.person_id
       LEFT JOIN health_care_events e ON e.care_event_id = r.care_event_id
      WHERE j.filed_at IS NOT NULL AND r.deleted = 0 AND ${s.sql}
      ORDER BY j.filed_at DESC LIMIT 30`
  ).bind(...s.binds).all();
  return c.json(results);
});
...
/**
 * Review: everything waiting on a person. Permanent, not an error path —
...
/**
 * File a document by hand. For handwritten notes and scans the reader could not
 * use: you supply who, when, what type and a summary, and the scan is filed and
 * findable. Structured values can be added later through the normal edit path.
 */
app.post('/api/core/uploads/:jobId/manual', async (c) => {
  const caller = c.get('caller');
  if (caller.role === 'viewer') throw new HttpError(403, 'Read-only access.');

  const jobId = c.req.param('jobId');
  const b = await c.req.json<{
    person: string; date: string; record_type: string; summary: string;
  ...
  if (!files.length) throw new HttpError(400, 'No document is attached.');

  await c.env.DB.prepare(
    `UPDATE core_jobs SET person_id = ?, care_event_id = ?, manual = 1, needs_attention = 0
      WHERE job_id = ?`
  ).bind(person, b.careEventId || null, jobId).run();
  ...
  await c.env.DB.prepare(
    `INSERT INTO core_audit_log (actor, action, ref_id, detail) VALUES (?, 'filed_manually', ?, ?)`
  ).bind(caller.email, out.recordId, summary.slice(0, 200)).run();
...
app.get('/api/health/export/workbook', async (c) => {
  const caller = c.get('caller');
  ...
  c.executionCtx.waitUntil(c.env.DB.prepare(
    `INSERT INTO core_audit_log (actor, action, detail) VALUES (?, 'export_workbook', ?)`
...
app.get('/api/health/export/documents', async (c) => {
  const caller = c.get('caller');
  ...
  c.executionCtx.waitUntil(c.env.DB.prepare(
    `INSERT INTO core_audit_log (actor, action, detail) VALUES (?, 'export_documents', ?)`
...
app.get('/api/core/documents/:documentId/file', async (c) => {
  const caller = c.get('caller');
  const doc = await c.env.DB.prepare(
    `SELECT d.*, p.name person FROM core_documents d JOIN core_people p USING (person_id)
...
app.route('/api/health/records', records);
app.route('/api/health/events', events);
app.route('/api/core/care', care);

/* ------------------------------------------------------------------ */
/* 3d. Profile, search, dashboard, documents, handout                  */
/* ------------------------------------------------------------------ */

app.get('/api/health/profile', async (c) => {
  const person = requirePerson(c.get('caller'), c.req.query('person'));
  const row = await c.env.DB.prepare(`SELECT * FROM core_profiles WHERE person_id = ?`).bind(person).first();
  return c.json(row ?? { person_id: person });
});

app.put('/api/health/profile', async (c) => {
  const caller = c.get('caller');
  if (caller.role === 'viewer') throw new HttpError(403, 'Read-only access.');
  ...
  await c.env.DB.prepare(
    `INSERT INTO core_profiles (person_id, date_of_birth, blood_group, allergies, chronic_conditions,
...
app.get('/api/health/search', async (c) => {
  const person = requirePerson(c.get('caller'), c.req.query('person'));
    ...
    c.env.DB.prepare(
      `SELECT t.parameter, t.result_text, t.unit_raw, t.test_date, t.flag, p.name person
        ...
        WHERE t.deleted = 0 AND t.is_abnormal = 1 AND ${s2.sql}
          AND t.test_date = (SELECT MAX(t2.test_date) FROM health_test_results t2
...
app.get('/api/health/records/:recordId/documents', async (c) => {
...
app.get('/api/health/handout', async (c) => {
  const person = requirePerson(c.get('caller'), c.req.query('person'));
...
app.post('/api/health/handout/share', async (c) => {
  const caller = c.get('caller');
  if (caller.role === 'viewer') throw new HttpError(403, 'Read-only access.');
  ...
  await c.env.DB.prepare(
    `INSERT INTO core_shares (share_id, person_id, kind, r2_key, created_by, expires_at, max_views)
     VALUES (?,?,'handout','',?,?,20)`
...
app.get('/s/:shareId', async (c) => {
  const share = await c.env.DB.prepare(
    `SELECT * FROM core_shares WHERE share_id = ? AND revoked = 0`
...
async function setting(env: Env, key: string): Promise<string> {
  const row = await env.DB.prepare(`SELECT value FROM core_settings WHERE key = ?`).bind(key).first<{ value: string }>();
  return row?.value ?? '';
}
...
export default {
  ...
  async queue(batch: MessageBatch<OcrMessage>, env: Env) {
    for (const msg of batch.messages) {
      ...
      try {
        ...
        // You said whose it was, but the page says someone else you also have
        // on file. Your choice stands; the document stops so you can look.
        if (job?.person_id && job.person_explicit && extraction.patient_name) {
          const printed = await matchPerson(env, extraction.patient_name);
          if (printed && printed !== job.person_id) {
            await env.DB.prepare(
              `UPDATE core_jobs SET auto_file = 0, needs_attention = 1, detected_name = ?
                WHERE job_id = ?`
        ...
        await env.DB.prepare(
          `UPDATE core_job_files SET ai_status='done', ai_json=?, last_error='' WHERE job_file_id = ?`
        ).bind(JSON.stringify(extraction), jobFileId).run();

        // Only assemble the review once every file in the batch has been read.
        const pending = await env.DB.prepare(
          `SELECT COUNT(*) n FROM core_job_files WHERE job_id = ? AND ai_status <> 'done'`
        ).bind(jobId).first<{ n: number }>();

        if (!pending?.n) {
          const { results } = await env.DB.prepare(
            `SELECT ai_json FROM core_job_files WHERE job_id = ? ORDER BY file_index`
  ...
  async scheduled(_event: ScheduledController, env: Env) {
    try {
      ...
      await env.DB.prepare(
        `UPDATE core_shares SET revoked = 1 WHERE revoked = 0 AND expires_at < datetime('now')`
      ).run();

      // Keep the audit log from growing without limit.
      await env.DB.prepare(
        `DELETE FROM core_audit_log WHERE id < (SELECT MAX(id) - 20000 FROM core_audit_log)`
      ).run();
    } catch (err) {
